...
exports.LiveMeetingManager = void 0;
const events_1 = require("events");
const AgentCommunicationBus_1 = require("./AgentCommunicationBus");
const AgentInterfaces_1 = require("./AgentInterfaces");
class LiveMeetingManager extends events_1.EventEmitter {
    constructor() {
        super();
        this.activeSessions = new Map();
        this.meetingHistory = new Map();
        this.communicationBus = new AgentCommunicationBus_1.AgentCommunicationBus();
        this.agents = new Map();
        this.initializeAgents();
        this.setupEventListeners();
    }
    initializeAgents() {
        const claudeAgent = new AgentInterfaces_1.ClaudeAgent();
        const kiroAgent = new AgentInterfaces_1.KiroAgent();
        const geminiAgent = new AgentInterfaces_1.GeminiAgent();
        const amazonQAgent = new AgentInterfaces_1.AmazonQAgent();
    ...
    async startLiveMeeting(topic, participants, meetingType = 'brainstorm', context) {
        console.log(`🏛️ Starting live multi-agent meeting`);
        console.log(`📋 Topic: ${topic}`);
        console.log(`👥 Participants: ${participants.join(', ')}`);
        console.log(`🎯 Type: ${meetingType}`);
        // Create collaboration session
        const sessionId = await this.communicationBus.createCollaborationSession(topic, participants, meetingType, {
            topic,
    ...
    async initializeMeetingFacilitator(sessionId) {
    ...
    async evaluateAndUpdateConsensus(sessionId) {
    ...
    async facilitateNextResponse(sessionId) {
        const session = this.communicationBus.getSessionState(sessionId);
        const history = this.meetingHistory.get(sessionId) || [];
    ...
    async requestAgentResponse(sessionId, agentId) {
    ...
    async addUserMessage(sessionId, content, directTo) {
        const userMessage = {
            id: `user_${Date.now()}`,
            from: {
    ...
    async startPredefinedScenario(scenarioIndex) {
        const scenarios = this.getPredefinedScenarios();
        const scenario = scenarios[scenarioIndex];
        if (!scenario) {
            throw new Error(`Scenario ${scenarioIndex} not found`);
        }
        return await this.startLiveMeeting(scenario.context.topic, scenario.expectedParticipants, 'problem_solving', scenario.context);
    }
}
exports.LiveMeetingManager = LiveMeetingManager;
//# sourceMappingURL=LiveMeetingManager.js.map