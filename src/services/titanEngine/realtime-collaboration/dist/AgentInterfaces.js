...
exports.AmazonQAgent = exports.GeminiAgent = exports.KiroAgent = exports.ClaudeAgent = void 0;
class ClaudeAgent {
    constructor() {
        this.agentId = 'claude';
        this.capabilities = {
    ...
    async generateResponse(context, previousMessages) {
...
${technicalAnalysis.recommendations.map(r => `• ${r}`).join('\n')}`,
    ...
    async analyzeTechnicalRequirements(context, messages) {
        return {
    ...
    async provideFeedback(topic, messages) {
        return {
            agreement: 0.85,
    ...
    async suggestCollaborators(topic) {
    ...
    async evaluateConsensus(discussion) {
        return {
...
${culturalAnalysis.recommendations.map(r => `• ${r}`).join('\n')}`,
    ...
    async assessCulturalImplications(context, messages) {
        return {
    ...
    async evaluateConsensus(discussion) {
        const culturalConsiderations = discussion.filter(msg => msg.content.toLowerCase().includes('cultural') ||
            msg.content.toLowerCase().includes('community')).length;
...
${statisticalAnalysis.validation.map(v => `• ${v}`).join('\n')}`,
    ...
    async analyzeDataPatterns(context, messages) {
        return {
...
${businessAnalysis.scalability.map(s => `• ${s}`).join('\n')}`,
    ...
    async analyzeBusinessImplications(context, messages) {
        return {
...
exports.AmazonQAgent = AmazonQAgent;
//# sourceMappingURL=AgentInterfaces.js.map