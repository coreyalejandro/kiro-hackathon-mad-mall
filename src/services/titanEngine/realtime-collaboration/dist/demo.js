...
exports.runDemoMeeting = runDemoMeeting;
...
async function runDemoMeeting() {
    ...
    try {
        ...
        console.log('============================================================');
        const sessionId = await meetingManager.startLiveMeeting('AI-powered wellness goal tracking for Black women with Graves\' disease', ['claude', 'kiro', 'gemini', 'amazon_q'], 'brainstorm', {
            objectives: [
        ...
        await meetingManager.addUserMessage(sessionId, 'I want this feature to feel supportive, not judgmental. How can we ensure the goals feel like community encouragement rather than clinical tracking?', ['kiro', 'claude']);
        // Allow responses
        await new Promise(resolve => setTimeout(resolve, 10000));
        // Ask Gemini for data validation
        await meetingManager.addUserMessage(sessionId, 'Gemini, how would you statistically validate that our approach is more effective than traditional individual goal tracking?', ['gemini']);
        await new Promise(resolve => setTimeout(resolve, 8000));
        // Ask Amazon Q about business implications
        await meetingManager.addUserMessage(sessionId, 'Amazon Q, what are the business and operational considerations for implementing this community-focused approach?', ['amazon_q']);
        await new Promise(resolve => setTimeout(resolve, 8000));
