...
import {
  ...
  ConnectionRecommendationSchema,
} from '../types/recommendation-types';
import { AgentConfig, AgentContext, AgentResponse } from '../types/agent-types';

export class RecommendationAgent extends AbstractBaseAgent<
  RecommendationRequest,
  RecommendationResult
> {
  ...
  validateInput(input: unknown): RecommendationRequest {
    return RecommendationRequestSchema.parse(input);
  }
  ...
  protected async processInput(
    ...
    try {
      ...
      const result = this.parseStructuredResponse<RecommendationResult>(
        response,
        RecommendationResultSchema
      );
...
5. Respectful of privacy and confidentiality`;
  ...
  private createRecommendationPrompt(input: RecommendationRequest): string {
...
6. Balance popular and niche suggestions`;
  ...
  async recommendConnections(
    ...
    const systemPrompt = this.createSystemPrompt(
      'You are a connection matching specialist. Find compatible users for meaningful peer support connections.',
      context
    );
...
[
  {
    ...
    try {
      ...
      return Array.isArray(connections) ? connections.map(conn => 
        ConnectionRecommendationSchema.parse(conn)
      ) : [];
    } catch (error) {
      return [];
    }
  ...
  async recommendWellnessActivities(
    userProfile: UserProfile,
  ...
  async explainRecommendation(
    ...
    const systemPrompt = this.createSystemPrompt(
      'You are a recommendation explainer. Provide clear, personalized explanations for why specific recommendations were made.',
      context
    );
