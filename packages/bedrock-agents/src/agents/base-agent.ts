...
export abstract class AbstractBaseAgent<TInput = any, TOutput = any> implements BaseAgent<TInput, TOutput> {
  protected readonly bedrockClient: BedrockRuntimeClient;
  public readonly config: AgentConfig;

  constructor(config: AgentConfig, region: string = 'us-east-1') {
    this.config = config;
  ...
  protected async invokeBedrockModel(
    prompt: string,
    systemPrompt?: string
